...
(function initSkillGraph() {
    ...
    // Load data from skill-graph-data.js module
    const skillData = window.skillGraphData;
    const skills = skillData.skills;
    const moonOrbits = skillData.moonOrbits;
    const subMoonOrbits = skillData.subMoonOrbits;
    const getSunPosition = skillData.getSunPosition;
    const getSolarSystemTilt = skillData.getSolarSystemTilt;
    ...
    // Reference spaceParticleParams and volumetricParams from core.js
    ...
    function renderSpheresGL(nodes, hovered, connected) {
        ...
        gl.uniform1f(sphereProgram.uMouseLightEnabled, 0.0); // Mouse light feature removed
    ...
    function resize() {
        ...
        // Place non-sun nodes randomly in sphere (orbital positions calculated in simulate())
        nodes.forEach(node => {
            if (!node.placed) {
    ...
    // ========================================
    // INITIALIZE UI PANELS AND SETTINGS (from modules)
    // ========================================

    // Set up callbacks for UI panels module to access nodes
    if (window.skillGraphUI) {
        window.skillGraphUI.setGetNodesCallback(function() { return nodes; });
        window.skillGraphUI.initAllPanels();
    }

    // Respawn stars with current parameters (count, distance)
    // Kept here because it needs access to local WebGL variables
    ...
    // Expose respawnStars globally for settings panel
    window.respawnStars = respawnStars;

    // Set up callbacks for settings module
    if (window.skillGraphSettings) {
        window.skillGraphSettings.setRespawnStarsCallback(respawnStars);
        window.skillGraphSettings.setUpdateLightFromKelvinCallback(window.skillGraphUI ? window.skillGraphUI.updateLightFromKelvin : null);
        window.skillGraphSettings.initAutoSave();
        window.skillGraphSettings.createSettingsButtons();
        window.skillGraphSettings.load();
    }
